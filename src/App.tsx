  ...
  return (
    <div className="min-h-screen bg-paper bg-grain-overlay">
      <div className="max-w-5xl mx-auto px-5 py-10 sm:py-14">
        ...
        {view.screen === 'exam-setup' && (
          <ExamSetup onStart={startExam} onBack={goHome} />
        )}

        {view.screen === 'exam-run' && (
          <ExamRunner
            key={view.steps.map((s) => s.exerciseId).join('-')}
            config={view.config}
            steps={view.steps}
        ...
        {view.screen === 'writing-editor' && (
          <WritingEditor
            key={view.prompt.id}
            prompt={view.prompt}
            onBack={() => setView({ screen: 'writing-home' })}
          />
        )}

        <footer className="mt-16 pt-6 border-t border-ink/10 text-xs text-inkSoft font-mono">
